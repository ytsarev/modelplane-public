...
# `nix run .#e2e-local` (which provides the tooling and the Nix-built function
...
# Remove the temp files/dirs created below (init resources, the isolated Docker
# config, the rendered manifests) on any exit, so a run doesn't leak them.
cleanup() {
	# Only remove paths we actually created, so an unset var never reaches
	# `rm -rf`. Each is an mktemp path under $TMPDIR.
	if [ -n "${init:-}" ]; then rm -f "$init" || true; fi
	if [ -n "${docker_config:-}" ]; then rm -rf "$docker_config" || true; fi
	# --no-apply hands $rendered to the user to apply after the script exits, so
	# keep it then; otherwise project run has already consumed it.
	if [ -n "${rendered:-}" ] && [ "${keep_rendered:-0}" != 1 ]; then rm -rf "$rendered" || true; fi
}
trap cleanup EXIT
...
if kind get clusters 2>/dev/null | grep -qx "$WL"; then
	...
	*)
		echo "workload cluster $WL is ${ver:-unreachable}, but v1.34 is required for the DRA APIs; recreate it with: nix run .#e2e-local -- --clean" >&2
		exit 1
		;;
	esac
else
...
SUBNET="$(docker network inspect kind -f '{{range .IPAM.Config}}{{println .Subnet}}{{end}}' | grep -E '^[0-9]+\.' | head -1)"
...
kubectl --context "$WLCTX" apply -f "$ROOT/hack/local-e2e/dra-example-driver.yaml"
...
# workload cluster. --internal gives an address reachable from the control
# plane's provider pods across the shared kind Docker network.
log "Building init resources: RBAC prerequisites + lean-control-plane config + workload kubeconfig Secret"
init="$(mktemp)"
{
	# The getting-started's RBAC prerequisites (+ the modelplane-system namespace).
	# Crossplane needs these permissions BEFORE it composes native cluster
	# resources (Gateway API, MetalLB, Service/EndpointSlice) on the control plane;
	# without them the InferenceGateway wedges on "failed waiting for informer to
	# sync" — crossplane can't watch the Namespace the IG composes.
	#
	# Strip prerequisites.yaml's leading comment-only document: `crossplane project
	# run --init-resources` rejects it ("invalid or empty raw resource") where
	# kubectl would skip it. awk drops leading comment/blank/`---` lines and prints
	# from the first real resource line on (inline comments in resources are fine).
	awk 'started { print; next }
       /^[[:space:]]*#/ || /^[[:space:]]*$/ || /^---[[:space:]]*$/ { next }
       { started = 1; print }' \
		"$ROOT/docs/manifests/getting-started/prerequisites.yaml"
	# Trim the control plane to the providers the BYO scenario uses: a narrowed
	# MRAP plus scale-to-0 for the unused cloud providers (see
	# lean-control-plane.yaml). Applied before the config's dependencies install.
	printf '\n---\n'
	cat "$ROOT/hack/local-e2e/lean-control-plane.yaml"
...
#   --no-apply  install the config only (no manifests), so project run returns
#               once the config is healthy — for gradual, manual apply/debugging.
...
rendered="$(mktemp -d)"
cp "$ROOT/hack/local-e2e/manifests/"*.yaml "$rendered/"
sed -i.bak "s/172\.18\.255/${PREFIX}.255/g" "$rendered/"*.yaml && rm -f "$rendered/"*.bak
extra=(--extra-resources "$rendered")
verify=0
case "${1:-}" in
--no-apply)
	extra=()
	keep_rendered=1
	log "--no-apply: config only; apply manifests from $rendered (kept for you)"
	;;
--verify) verify=1 ;;
esac
...
if [ "$verify" = 0 ]; then
	log "Done. Curl the ModelService per the README; clean up with: nix run .#e2e-local -- --clean"
	exit 0
fi
