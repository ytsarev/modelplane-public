...
- Everything else — `kind`, `kubectl`, `curl`, `git`, and the `crossplane` CLI —
  is provided by the flake via `nix run .#e2e-local`.
...
```bash
nix run .#e2e-local              # bring up both clusters + deploy the mock model
nix run .#e2e-local -- --verify  # same, then wait for readiness and assert a live 200
nix run .#e2e-local -- --clean   # tear both clusters down
```
...
`nix run .#e2e-local` materialises the Nix-built function images and hands off to
...
2. Install MetalLB on it (the serving stack doesn't) with a pool inside the
   ...
   **dra-example-driver** (fake GPUs), and label its node for the `gpu-synthetic`
   pool.
3. Build a kubeconfig Secret for the workload cluster (`kind get kubeconfig
   --internal`, reachable from control-plane pods over the shared kind network).
4. `crossplane project run` for the **control plane**, with that Secret as
   `--init-resources` and the (subnet-templated) Modelplane manifests as
   `--extra-resources`.
...
hack/local-e2e/
  ...
  manifests/                 # --extra-resources on the control plane
    00-namespaces.yaml
